...
// The frog that is trying to get across the highway.  It moves in the direction of
// mouse clicks.  It is killed when a vehicle hits it and is reincarnated when the user
// restarts.
public class Frog
{
        // The location where the frog starts when trying to cross the highway.  We remember
        // this so we can reincarnate the frog.
        double startX;
        double startY;
   ...
   public Frog(  Image      frogPic, int      highwayCenterX,  int     highwayBottom, int     laneWidth,
   ...
   public boolean overlaps(VisibleImage anImage) {
                   if ( frogImage          .overlaps( anImage       ))
                      return true  ;
                    else
                      return false;
   }

   // Displays the ouch text and causes the frog to not move until after it is reincarnated.
   //
   ...
   // Brings the frog back to life.  Removes the ouch text and moves the frog back to the
   // start
   //
   ...
   // Causes the frog to move one hop towards the point where the user clicked.  Left/right
   // moves are given preference over forward/back moves.
   //
   // Precondition:  The frog is alive.
